...
<script>
    import { ElForm, ElFormItem, ElInput, ElButton, ElSelect, ElOption, ElMessage, ElDialog } from 'element-plus';

    export default {
        components: {
            ElForm, ElFormItem, ElInput, ElButton, ElSelect, ElOption, ElMessage, ElDialog
        },
        data() {
            return {
                loan_data: {
        ...
        methods: {
                ...
                this.$refs.loan_form.validate(async (valid) => {
                    if (valid) {
                        try {
                            ...
                            const response = await this.$axios.post('/add-loan', loanData);
            ...
            resetForm() {
                ...
                this.$refs.loan_form.resetFields(); // 重置表单验证状态
            }
        }
    }
</script>
